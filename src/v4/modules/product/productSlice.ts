...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    addViewedProduct: (state, { payload }: { payload: Product }) => {
      const index = state?.viewedProducts?.findIndex((item) => item.id === payload?.id)
      if (index !== -1) {
        state.viewedProducts = state.viewedProducts.filter((item) => item.id !== payload.id)
      }

      state.viewedProducts.unshift(payload)

      if (state.viewedProducts?.length > 12) {
        state.viewedProducts = state.viewedProducts.filter((item) => item.id !== state.viewedProducts[12].id)
      }
    }
  }
})
